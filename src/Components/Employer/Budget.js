...
export const EmployerBudgetPage = () => {
    ...
    const deleteBudget = (id) => {
        ...
        try {
            axios.delete(`${process.env.REACT_APP_API_BASE_URL}/budget/${id.target.value}`, { headers: headers })
                    ...
                    <main className="main-content position-relative max-height-vh-100 h-100 mt-1 border-radius-lg ">
                        ...
                        <div className="container-fluid py-4">
                                        ...
                                        <div className="card-header pb-0">
                                            <div className="row">
                                                <div class="col-12 text-end ">
                                                    <button onClick={addBudget} className="btn btn-link btn-info text-secondary text-white"><a className="fa fa-plus text-xs text-white" data-bs-toggle="modal" ></a> <span class="ms-1">Add Budget</span></button>
                                                </div>
                                            </div>
                                        </div>
                          ...
                          ">Actions</th>
                                                    ...
                                                    <tbody>
                                                        {
                                                            budgetList && budgetList.length > 0 ? budgetList.map(budget =>
                                                                <tr>
                                                                    <td>
                                                                        <p className="text-sm text-center font-weight-bold mb-0">{budget.amount}</p>
                                                                    </td>
                                                                    <td>
                                                                        <p className="text-sm text-center font-weight-bold mb-0">{budget.budget_type}</p>
                                                                    </td>
                                                                    <td>
                                                                        <p className="text-sm text-center font-weight-bold">{budget.employee_name}</p>
                                                                    </td>
                                                                    <td className="align-middle  text-center text-sm">
                                                                        <button onClick={editBudget} className="fas fa-edit btn btn-link text-secondary mb-0" style={{ marginRight: "10px" }} value={budget.id} title="Edit Budget"></button>
                                                                        <button onClick={deleteBudget} className="fa fa-trash btn btn-link text-secondary mb-0" value={budget.id} title="Edit Budget"></button>
                                                                    </td>
                                                                </tr>
                                                            ) : (
                                                                <tr>
