...
export const AdminEmployersPage = () => {
  ...
  const deleteCompany = (id) => {
    ...
    try {
      axios.delete(`${process.env.REACT_APP_API_BASE_URL}/employer/${id.target.value}`, { headers: headers })
                      ...
                      <div className="card-header pb-0">
                        <div className="row">
                          <div class="col-12 text-end ">
                            <button onClick={addCompany} className="btn btn-link btn-info text-secondary text-white"><a className="fa fa-plus text-xs text-white" data-bs-toggle="modal" ></a> <span class="ms-1">Add Employer</span></button>
                          </div>
                        </div>
                      </div>
                      <div className="card-body px-0 pt-0 pb-2">
                        <div className="table-responsive p-0">
                          ...
                          ">Actions</th>
                            ...
                            <tbody>
                              {
                                employerList && employerList.length > 0 ? employerList.map(employer =>
                                  <tr>
                                    <td>
                                      <p className="text-sm text-center font-weight-bold mb-0">{employer.name}</p>
                                    </td>
                                    <td>
                                      <p className="text-sm text-center font-weight-bold mb-0">{employer.user.email}</p>
                                    </td>
                                    <td>
                                      <p className="text-sm text-center font-weight-bold">{employer.country}</p>
                                    </td>
                                    <td className="align-middle  text-center text-sm">
                                      <button onClick={editCompany} className="fas fa-edit btn btn-link text-secondary mb-0" style={{ marginRight: "10px" }} value={employer.id} title="Edit Employer"></button>
                                      <button onClick={deleteCompany} className="fa fa-trash btn btn-link text-secondary mb-0" value={employer.id} title="Edit Employer"></button>
                                    </td>
                                  </tr>
                                ) : (
                                  <tr>
