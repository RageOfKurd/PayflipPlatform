...
export const AdminEmployeesPage = () => {
  ...
  const deleteEmployee = (id) => {
    ...
    try {
      axios.delete(`${process.env.REACT_APP_API_BASE_URL}/employee/${id.target.value}`, { headers: headers })
                    ...
                    <div className="card-header pb-0">
                      <div className="row">
                        <div class="col-12 text-end ">
                          <button onClick={addEmployee} className="btn btn-link btn-info text-secondary text-white"><a className="fa fa-plus text-xs text-white" data-bs-toggle="modal" ></a> <span class="ms-1">Add Employee</span></button>
                        </div>
                          ...
                          <tbody>
                            {
                              employeeList && employeeList.length > 0 ? employeeList.map(employee =>
                                <tr>
                                  <td>
                                    <p className="text-sm text-center font-weight-bold mb-0">{employee.name}</p>
                                  </td>
                                  <td>
                                    <p className="text-sm text-center font-weight-bold mb-0">{employee.user.email}</p>
                                  </td>
                                  <td>
                                    <p className="text-sm text-center font-weight-bold  mb-0">{employee.country}</p>
                                  </td>
                                  <td className="align-middle  text-center text-sm">
                                    <button onClick={editEmployee} className="fas fa-edit btn btn-link text-secondary mb-0" style={{ marginRight: "10px" }} value={employee.id} title="Edit Employee"></button>
                                    <button onClick={deleteEmployee} className="fa fa-trash btn btn-link text-secondary mb-0" value={employee.id} title="Delete Employee"></button>
                                  </td>
                                </tr>
                              ) : (
                                <tr>
